    ...
    const vector<vector<MapPoint*>>& vvpMatches)
{
    ...
    for(size_t idx = 0; idx < vvpMatches.size(); idx++)
    {
        ...
        for(size_t i = 0; i < vpMatches.size(); i++)
        {
            ...
            vDepthRefs.push_back({idx, i, depth});
        }
    }
    ...
    cout << "global depth stats: min " << PercentileDepth(vDepthRefs, 0.0f)
...
void CalibC2C::RunCalib()
{
    ...
    vector<vector<MapPoint*>> mmpsSelected = SelectGlobalDepthBalancedMatches(rsrc, rdst, mmps);
    
    cout << "matched kfs selected for global optim size: " << rsrc.size() << endl;
    cout << "matched mps from CamMap matching size: " << CountMatches(mmps) << endl;
    cout << "matched mps selected for optim size: " << CountMatches(mmpsSelected) << endl;
    ...
    int inliers = OptimizeSim3ForCalibr(rsrc, rdst, mmpsSelected, firstPose, 10, bFixScale);
    ...
    inliers = OptimizeSim3ForCalibr(rsrc, rdst, mmpsSelected, finalPose, 10, bFixScale, finalPose1, finalPose2);
    cout << "inliers size: " << inliers << endl;
