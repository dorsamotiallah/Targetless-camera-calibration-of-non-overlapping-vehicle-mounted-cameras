...
float PercentileDepth(const vector<DepthMatchRef>& vDepthRefs, const float percentile)
{
    if(vDepthRefs.empty())
        return 0.0f;
                ...
                if(numProjMatches >= 25)
                {
                    ...
                    if(numOptMatches >= 10)
                    {
                        ...
                        if(numProjMatches >= 40)
                        {
                            ...
                            vector<MapPoint*> vpBalancedMatchedMPs = SelectDepthBalancedMatches(pKF, pMostBoWMatchesKF, vpMatchedMPs);
                            const int balancedProjMatches = CountMatches(vpBalancedMatchedMPs);

                            // 8. save best result along the iteration
                            if(nBestMatchesReproj < balancedProjMatches)
                            {
                                nBestMatchesReproj = balancedProjMatches; 
...
void CalibC2C::ComputeGlobalDepthThresholds()
{
    vector<float> vDepths;

    for(KeyFrame* pKF : srcKFs)
    {
        if(!pKF || pKF->isBad())
            continue;
        ...
        for(MapPoint* pMP : vpMapPoints)
        {
            ...
            const float depth = Pc.z();
            if(std::isfinite(depth) && depth > 0.0f)
                vDepths.push_back(depth);
        }
    }

    if(vDepths.size() < static_cast<size_t>(minDepthBalancedMatches))
    {
        bDepthBalancedThresholdsReady = false;
        cout << "global adaptive depth thresholds skipped: valid " << vDepths.size() << endl;
        return;
    }

    std::sort(vDepths.begin(), vDepths.end());
    const size_t nTotal = vDepths.size();
    const size_t nNearIdx = std::max(static_cast<size_t>(1), nTotal / 3) - 1;
    const size_t nFarIdx = std::min((2 * nTotal) / 3, nTotal - 1);

    depthBalancedNearThreshold = vDepths[nNearIdx];
    depthBalancedFarThreshold = vDepths[nFarIdx];
    bDepthBalancedThresholdsReady = true;

    cout << "global adaptive depth thresholds: valid " << nTotal
         << ", near threshold " << depthBalancedNearThreshold
         << ", far threshold " << depthBalancedFarThreshold
         << ", min " << vDepths.front()
         << ", max " << vDepths.back() << endl;
}

vector<MapPoint*> CalibC2C::SelectDepthBalancedMatches(KeyFrame* pKF1, KeyFrame* pKF2, const vector<MapPoint*>& vpMatches)
{
    if(!bUseDepthBalancedSelection || !bDepthBalancedThresholdsReady || !pKF1 || !pKF2)
        return vpMatches;
    ...
    vector<DepthMatchRef> vDepthRefs;
    ...
    for(size_t i = 0; i < vpMatches.size(); i++)
    {
        ...
        const float depth = Pc1.z();
        if(!std::isfinite(depth) || depth <= 0.0f)
            continue;

        vDepthRefs.push_back({i, depth});
    }

    if(vDepthRefs.size() < static_cast<size_t>(minDepthBalancedMatches))
    {
        cout << "local adaptive depth selection skipped: valid " << vDepthRefs.size() << endl;
        return vpMatches;
    }

    const size_t nTotal = vDepthRefs.size();
    std::array<vector<DepthMatchRef>, 3> vDepthGroups;
    for(const DepthMatchRef& ref : vDepthRefs)
    {
        if(ref.depth <= depthBalancedNearThreshold)
            vDepthGroups[0].push_back(ref);
        else if(ref.depth >= depthBalancedFarThreshold)
    ...
    {
        cout << "local adaptive depth selection skipped: near " << nNear
             << ", middle " << nMiddle
             << ", far " << nFar << endl;
        return vpMatches;
    }

    vector<MapPoint*> vpBalancedMatches(vpMatches.size(), nullptr);
    std::mt19937 rng(static_cast<unsigned int>((pKF1->mnId + 1) * 73856093u
                                             ^ (pKF2->mnId + 1) * 19349663u
                                             ^ nTotal * 83492791u));
    for(vector<DepthMatchRef>& vGroup : vDepthGroups)
    {
        std::shuffle(vGroup.begin(), vGroup.end(), rng);
        for(size_t i = 0; i < nPerGroup; i++)
        {
            const DepthMatchRef& ref = vGroup[i];
            vpBalancedMatches[ref.matchIdx] = vpMatches[ref.matchIdx];
        }
    }

    cout << "global-threshold depth selection: valid " << nTotal
         << ", near " << nNear
         << ", middle " << nMiddle
         << ", far " << nFar
         << ", selected per group " << nPerGroup
         << ", selected " << CountMatches(vpBalancedMatches) << endl;
    std::sort(vDepthRefs.begin(), vDepthRefs.end(), [](const DepthMatchRef& a, const DepthMatchRef& b) {
        return a.depth < b.depth;
    });
    cout << "local depth stats: min " << PercentileDepth(vDepthRefs, 0.0f)
         << ", global near threshold " << depthBalancedNearThreshold
         << ", median " << PercentileDepth(vDepthRefs, 0.50f)
         << ", global far threshold " << depthBalancedFarThreshold
         << ", max " << PercentileDepth(vDepthRefs, 1.0f) << endl;

    return vpBalancedMatches;
}

void CalibC2C::RunCalib()
{
    vector<KeyFrame*> rsrc, rdst;
    vector<vector<MapPoint*>> mmpsSelected;
    vector<vector<MapPoint*>> mmpsAllDepth;

    int bestMatchesNum = 0;
    g2o::Sim3 g2oS12;

    ComputeGlobalDepthThresholds();
    ...
    for(auto&& kf : srcKFs)
    {
        ...
        if(valid){
            vector<MapPoint*> vpBalancedMatchedMPs = SelectDepthBalancedMatches(kf, matchedKF, vpMatchedMPs);
            matchesNum = CountMatches(vpBalancedMatchedMPs);
            ...
            mmpsSelected.push_back(vpBalancedMatchedMPs);
            mmpsAllDepth.push_back(vpMatchedMPs);
        }
    }
    ...
    cout << "matched mps selected for keyframe scoring size: " << CountMatches(mmpsSelected) << endl;
    cout << "matched mps all-depth for optim size: " << CountMatches(mmpsAllDepth) << endl;
    ...
    int inliers = OptimizeSim3ForCalibr(rsrc, rdst, mmpsAllDepth, firstPose, 10, bFixScale);
    ...
    inliers = OptimizeSim3ForCalibr(rsrc, rdst, mmpsAllDepth, finalPose, 10, bFixScale, finalPose1, finalPose2);
    cout << "inliers size: " << inliers << endl;
